...
resources = ['Coal and coal products', 'Oil products', 'Natural gas', 'Biofuels and waste',
'Electricity',  'Heat']
...
for k in dfs:
    ...
    df['Biofuels and waste (%)'] = df['Biofuels and waste'] * 0.35 / df['Useful consumption']
    ...
    df1 = df[['Coal and coal products (%)', 'Oil products (%)', 'Natural gas (%)', 'Biofuels and waste (%)', 'Electricity (%)', 'Heat (%)']]
    df1.to_excel(writer, sheet_name=str(k), index=False, startcol=15)  # Записываем датафрейм в файл.
    country.to_excel(writer, sheet_name=str(k), index=False, startcol=14)
    if k == '2019':
        break
